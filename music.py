...
class Player(commands.Cog):
    ...
    async def skip(self, ctx):
        ...
        if role in ctx.author.roles:
            ...
            rembed = discord.Embed(title="A DJ skipped the song", description=f'{ctx.author} has skipped the song')
    ...
    @commands.command()
    async def clearqueue(self, ctx): # display the current guilds queue
        if len(self.song_queue[ctx.guild.id]) == 0:
...
bot.run("OTQyMDE3ODU1NzMyNDY5ODIw.YgeYfg.n66dP0O4sTnoYvZMi5SC4CM3oh4")
